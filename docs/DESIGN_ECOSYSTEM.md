...
| **E9** | Distribución | `majestic` ARR sin bundling. `astral_core`, `expedition_core`, `almanac_core`, GeckoLib y (desde 2026-09-24) `vellumli` se declaran como **required dependencies** en la página de CurseForge y en `neoforge.mods.toml` de `majestic`. `jei` = optional. En `almanac_core` (librería) `vellumli` sigue siendo optional. |
