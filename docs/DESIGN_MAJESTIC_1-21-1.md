...
| 2026-09-24 | **Jefe I combate v2 (beta.13)**: animaciones v2 del taller + 3 ataques con daño en el tick de impacto (servidor; las marcas `timeline` de GeckoLib son solo cliente), carrera y embestida avisada en fase 2, `MOVE_STATE` sincronizado. Delegado a OpenCode `deepseek-v4.1-flash` (sin cortes de sandbox con la regla de rutas internas), revisado sin cambios. |
| 2026-09-25 | **Journey I (beta.14)**: categoría "El Viaje" del libro (prólogo-acertijo + capítulos 1-2 por advancements `majestic:journey/chapter_1/2`), hojas `beginning_page`/`shrine_page` (`JournalPageItem`), 4 mobs nocturnos (`content/entity/night/`, spawn por biome modifier `night_mobs`), hoja del santuario en el cofre. Decisiones: hoja garantizada hasta leer el cap. 1; uso con el libro en el inventario; progreso solo de quien usa la hoja. Código y traducción `es_es` delegados a OpenCode `deepseek-v4.1-flash`; contenido del libro escrito por Claude. Pendiente taller: modelos de mobs, texturas de hojas, Santuario con un solo cofre. |
| 2026-09-25 | **Assets Journey I (beta.15)**: 4 mobs + 2 hojas + Santuario de un cofre traídos del taller (`e1d5454`); daño sincronizado al impacto (Vigía 6 ticks, Reptador 5; Cultista ya a 10) con `DelayedMeleeAttack`; picado de la Polilla detectado por movimiento (fase de ataque de `Phantom` es privada). Delegado a OpenCode. |
| 2026-09-25 | **Correcciones de la prueba del Viaje I (beta.16)**: altar visible (`getRenderShape` → `MODEL`), árbol de logros `majestic:journey/root` (tick al entrar) → `fallen_shrine` → `observatory` (`minecraft:location` por estructura) → `guide/warden_of_the_gate`, capítulos con `read_by_default`. Taller: `JOURNEY_I_TALLER_GUIDE_PART2.md` (agarre del báculo + animaciones más marcadas, Santuario y Observatorio más complejos, imágenes para el libro). Delegado a OpenCode. |
| 2026-09-25 | **Journey I parte 2 (beta.17)**: entregas A-D del taller (`bc538f5`): Guardián con puño y animaciones más marcadas, Santuario 15×12×15 (cofre en 7,1,13), Observatorio con entrada 13×18×11 (jigsaw sin cambios: size 5, max_distance 80), páginas `vellumli:image` en los capítulos 1-2 (en/es, delegado a OpenCode). |
